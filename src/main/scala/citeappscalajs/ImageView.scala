...
object ImageView {
	...
	def imageDiv = {
		...
		<p id="image_reportingCurrentUrn" class="app_reportingCurrentUrn"> { 
			ImageModel.displayUrn.bind match {
				case Some(u) => {
					u.toString
				}
				case _ => {
										...
										<p class={ s"image_mappedUrn image_roiGroup_${iroi.roiGroup} ${ImageModel.idForMappedUrn(idx)}"}
											...
											onmouseover={ event: Event => {
												...
												def mappedUrnSpan(u:Urn, imgU:Cite2Urn) = {
													{ u match {
														case CtsUrn(_) => {
															CiteLinks.textLinks(u.asInstanceOf[CtsUrn]).bind
														}
														case Cite2Urn(_) => {
															CiteLinks.objectLinks(None, u.asInstanceOf[Cite2Urn]).bind
														}
														case _ => {
															<a>Unknown</a>
														}
...
def thumbnailView(contextUrn:Option[Cite2Urn], propVal:Cite2Urn) = {
	...
	if (ImageModel.imgUseLocal.get){
		...
		var path:String = ""
		(rW * rH) match {
			case x if x < 0.25 => path = ImageController.getThumbPath(justUrn)
		...
		onScreenImg
			...
			val path:String = ImageController.getThumbPath(propVal)
			val onScreenImg = document.createElement("img").asInstanceOf[HTMLImageElement]
			onScreenImg.setAttribute("crossOrigin","Anonymous")
			onScreenImg.setAttribute("src",path)
			onScreenImg.setAttribute("class","object_imgThumb")
			onScreenImg
		}
	}
